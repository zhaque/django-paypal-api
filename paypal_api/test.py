...
class TestMassPay(unittest.TestCase):
	def setUp(self):
		email = API_TEST_CUSTOMER
		amt = '10.00'
		self.mass_pay = {
			'receivertype' : "EmailAddress",
			'l_email0' : email,
			'l_amt0' : amt,
			#'l_note0' : 'Here is a note',
			#'l_unique0' : unique_id,
			'emailsubject': 'Mass Pay Test',
			'currencycode': 'USD',
		}

	def test_mass_pay_raw(self):
		sale = do_mass_pay_raw(**self.mass_pay)
		self.assertTrue(sale.success)
		
	def test_mass_pay_simple(self):
		rec = [(TEST_CUSTOMER, '5.00')]
		sale = do_mass_pay(rec)
		self.assertTrue(sale.success)
